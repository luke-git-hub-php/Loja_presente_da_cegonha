...
$data=$_POST['data'];
$queryr=mysqli_query($conexao,"select * FROM itens_venda WHERE data_compra='$data'");
...
while($resultado=mysqli_fetch_assoc($queryr)){
$pdf->SetTextColor(255,50,0);
	 	$pdf->Cell(15,1,$resultado['nome_prod'],1,0,'C');
	 	$pdf->Cell(15,1,substr($resultado['data_compra'], 8, 2) . " / ". substr($resultado['data_compra'], 5, 2) . " / ". substr($resultado['data_compra'], 0, 4),1,1,'C');
	 	$pdf->SetTextColor(50,255,0);
